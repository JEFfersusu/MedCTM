...
class ConvBN(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1):
        ...
        self.conv = nn.Conv2d(
            ...
            stride, 
            (kernel_size - 1) // 2, 
            bias=False  # BatchNorm 通常不带 bias
        )
        self.bn = nn.BatchNorm2d(out_channels)  # 始终初始化 BN 层
        self.relu = nn.ReLU(inplace=True)
        
    def forward(self, x):
        x = self.conv(x)
        
        # 动态判断：如果 batch_size=1，跳过 BatchNorm
        if x.size(0) == 1:
            x = self.relu(x)  # 直接 ReLU
        else:
            x = self.bn(x)    # 正常走 BN + ReLU
            x = self.relu(x)
...
def mobilenetv4_conv_large(num_classes=6):
    """
    ...
    return MobileNetV4(block_specs, num_classes=num_classes)